    ...
    methods: {
        ...
        async responderSugestao(sugestao) {
          ...
          async respostaCard() {
            ...
            const perguntaid = this.$route.params.id;

            const respon = await axios.get('http://127.0.0.1:5000/perguntaid/' + perguntaid);
            
            this.perguntas.push(respon.data);
            this.data.push({ pergunta: respon.data});
            ...
            const source = axios.CancelToken.source();
                ...
                try {
                  const response = await axios.get('http://127.0.0.1:5000/resposta/' + respon.data, {
                  cancelToken: source.token,
                });
